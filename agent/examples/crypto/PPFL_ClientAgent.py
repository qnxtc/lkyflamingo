...
class PPFL_ClientAgent(Agent):
    ...
    def wakeup(self, currentTime):
        ...
        if not self.peer_exchange_complete:
            ...
            weight = getWeights(previous_weight=self.prevWeight, max_iter=self.max_logreg_iterations,
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        if msg.body['msg'] == "PEER_EXCHANGE":
            ...
            log_print("Client weights received for iteration {} by {}: {}", self.current_iteration, self.id,
                      self.prevWeight)

            # Client number 1 (arbitrary choice) records the shared learning progress each iteration
            # for later visualization and analysis.
            if self.id == 1:
                is_acc, is_mcc, is_f1, is_mse, is_auprc, oos_acc, oos_mcc, oos_f1, oos_mse, oos_auprc = reportStats(
                    self.prevWeight, self.current_iteration, self.X_train, self.y_train, self.X_test, self.y_test)
