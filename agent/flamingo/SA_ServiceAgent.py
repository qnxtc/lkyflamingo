...
class SA_ServiceAgent(Agent):
    ...
    def reconstruction(self, currentTime):
        ...
        for i in range(len(sum_df)):
            ...
            prg_mi_holder = ChaCha20.new(key=sum_df[0][i], nonce=nonce)
        ...
        if len(self.dec_target_pairwise) != 0:
            ...
            for i in range(len(sum_df)):
                ...
                prg_pairwise_holder = ChaCha20.new(key=sum_df[0][i], nonce=nonce)
                data = b"secr" * self.vector_len
        ...
        def _prepare_test_sequences(self):
            """预处理测试数据（与客户端一致）"""
            raw_data = np.load("pems04.npz")['data']
            # 标准化（使用客户端传来的参数）
