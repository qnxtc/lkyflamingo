...
class SA_ServiceAgent(Agent):
    ...
    def __init__(self, id, name, type,
        ...
        self.global_model = TrafficLSTM()  # 全局LSTM模型
        self.global_model.train()  # 保持训练模式
    ...
    def receiveMessage(self, currentTime, msg):
        ...
        if msg.body['msg'] == "VECTOR":
            ...
            if not self.check_model_config(client_config):
                raise ValueError(f"Client {sender_id} 模型结构不兼容")

            # 存储首次接收的模型配置
            if not hasattr(self, 'global_model_config'):
                self.global_model_config = client_config
                # 初始化全局模型
                self.global_model = TrafficLSTM(
                    input_size=client_config['input_size'],
                    hidden_size=client_config['hidden_size'],
                    num_layers=client_config['num_layers'],
                    output_size=client_config['output_size']
                )
            # ...其余原有代码不变...
    ...
    def check_model_config(self, config):
        ...
        if not hasattr(self, 'global_model_config'):
            return True
        return all([
            config['input_size'] == self.global_model.input_size,
            config['hidden_size'] == self.global_model.hidden_size,
            config['num_layers'] == self.global_model.num_layers,
            config['output_size'] == self.global_model.output_size
        ])
    ...
    def reconstruction(self, currentTime):
        ...
        for name, param in self.global_model.state_dict().items():
            ...
            tensor_data = float_vec[ptr:ptr + param_size].reshape(param.shape)
            state_dict[name] = torch.FloatTensor(tensor_data)
            ptr += param_size
        ...
        # with torch.no_grad():
        #     test_X = torch.FloatTensor(self.test_sequences)
        #     preds = self.global_model(test_X).numpy()
        #
        # # 3. 反标准化
        # preds = self._inverse_transform(preds)
        # true = self._inverse_transform(self.y_test_raw)  # 需要提前存储原始测试数据
        #
        # # 4. 计算指标
        # mae = np.mean(np.abs(preds - true))
        # rmse = np.sqrt(np.mean((preds - true) ** 2))
        # print(f"Global Model - MAE: {mae:.2f}, RMSE: {rmse:.2f}")
        #########新增的版1#########

        #########新增的#########
        # 改为时间序列预测评估：
        def evaluate_metrics(model, X_test, y_test, scaler_mean, scaler_scale):
            model.eval()
            with torch.no_grad():
            ...
            # 反标准化
            preds = preds * scaler_scale[0] + scaler_mean[0]
            y_true = y_test * scaler_scale[0] + scaler_mean[0]
        ...
        mae, rmse = evaluate_metrics(self.global_model,
                                     self.X_test,
                                     self.y_test,
                                     self.scaler_mean,
                                     self.scaler_scale)
        ...
        def _prepare_test_sequences(self):
            """预处理测试数据（与客户端一致）"""
            raw_data = np.load("F:\DATA\pycharm\lkyflamingo\dataset\PEMS04\PEMS04.npz")['data']
            # 标准化（使用客户端传来的参数）
            scaled_data = (raw_data - self.scaler_mean) / self.scaler_scale
            ...
            for i in range(len(scaled_data) - seq_length - pred_length):
                X_test.append(scaled_data[i:i + seq_length])
                y_test.append(scaled_data[i + seq_length:i + seq_length + pred_length, 0])

            self.test_sequences = np.array(X_test)
            self.y_test_raw = np.array(y_test)

        def _inverse_transform(self, data):
            """反标准化流量数据"""
            dummy = np.zeros((data.shape[0], 3))  # 假设特征维度为3
            dummy[:, 0] = data.flatten()
            dummy = dummy * self.scaler_scale + self.scaler_mean
            return dummy[:, 0]
        #########新增的#########
        ...
        for id in self.ids:
            ...
            vec = np.concatenate(weight_vector)
